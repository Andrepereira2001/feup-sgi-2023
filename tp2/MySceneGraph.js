...
export class MySceneGraph {
    ...
    parseView(viewsNode) {
        ...
        this.scene.selectedCamera = defaultView
        ...
        if (this.views[defaultView] === null)
            return `the default view of ID = ${defaultView} must be specified`
        if (numOfViews === 0)
    ...
    /**
     * Parse the scene transformation instruction
     * @param {*} instructions scene transformation instuctions
     ...
     * @param {animations block element} animationNodes
     */
    parseAnimations(animationNodes) {
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            for(let i = 0; i < grandChildren.length; i++) {
                ...
                const tempInstant = this.reader.getFloat(grandChildren[i], 'instant');
                if (tempInstant === null){
                    this.onXMLMinorError(`Instant not defined for keyFrame (conflict: ID = ${animationID})`)
                    continue;
                } else if(instant === null){
                    instant = tempInstant
                } else if(tempInstant <= instant) {
                    this.onXMLMinorError(`Instant must be ordered (conflict: ID = ${animationID})`)
                    continue;
                } else instant = tempInstant

                let keyFrameChildren = grandChildren[i].children
                let translation = []
                let rotation = []
                let scale = []
                if(keyFrameChildren.length != 5){
                    this.onXMLMinorError(`Invalid transformations provided in keyFrame instant ${instant} (conflict: ID = ${animationID})`)
                    continue;
                }

                if(keyFrameChildren[0].nodeName != "translation"){
                    this.onXMLMinorError(`unknown tag <${keyFrameChildren[0].nodeName}>`);
                    continue;
                }
                translation = this.parseCoordinates3D(keyFrameChildren[0], `Error in Keyframe ${animationID} instant ${instant}.`)

                if(keyFrameChildren[1].nodeName != "rotation"){
                    this.onXMLMinorError(`unknown tag <${keyFrameChildren[1].nodeName}>`);
                    continue;
                }
                let axis = this.reader.getString(keyFrameChildren[1], 'axis');
                if(axis === null || axis !== "z"){
                    this.onXMLMinorError(`Error in Keyframe ${animationID} instant ${instant} z-axis-mandatory>`);
                    continue;
                }
                let angle = this.reader.getFloat(keyFrameChildren[1], 'angle');
                if(!isNumeric(angle)){
                    this.onXMLMinorError(`Error in Keyframe ${animationID} instant ${instant} z-angle>`);
                    continue;
                }
                rotation[2] = angle

                if(keyFrameChildren[2].nodeName != "rotation"){
                    this.onXMLMinorError(`unknown tag <${keyFrameChildren[2].nodeName}>`);
                    continue;
                }
                axis = this.reader.getString(keyFrameChildren[2], 'axis');
                if(axis === null || axis !== "y"){
                    this.onXMLMinorError(`Error in Keyframe ${animationID} instant ${instant} y-axis-mandatory>`);
                    continue;
                }
                angle = this.reader.getFloat(keyFrameChildren[2], 'angle');
                if(!isNumeric(angle)){
                    this.onXMLMinorError(`Error in Keyframe ${animationID} instant ${instant} y-angle>`);
                    continue;
                }
                rotation[1] = angle

                if(keyFrameChildren[3].nodeName != "rotation"){
                    this.onXMLMinorError(`unknown tag <${keyFrameChildren[3].nodeName}>`);
                    continue;
                }
                axis = this.reader.getString(keyFrameChildren[3], 'axis');
                if(axis === null || axis !== "x"){
                    this.onXMLMinorError(`Error in Keyframe ${animationID} instant ${instant} x-axis-mandatory>`);
                    continue;
                }
                angle = this.reader.getFloat(keyFrameChildren[3], 'angle');
                if(!isNumeric(angle)){
                    this.onXMLMinorError(`Error in Keyframe ${animationID} instant ${instant} x-angle>`);
                    continue;
                }
                rotation[0] = angle

                if(keyFrameChildren[4].nodeName != "scale"){
                    this.onXMLMinorError(`unknown tag <${keyFrameChildren[4].nodeName}>`);
                    continue;
                }
                scale = this.parseCoordinates3DVersionS(keyFrameChildren[4], `Error in Keyframe ${animationID} instant ${instant}.`)

                let keyFrame = new MyKeyFrame(instant, translation, rotation, scale)
                keyFrames.push(keyFrame)
            }
            this.animations[animationID] = new MyKeyframeAnimation(this.scene, keyFrames)
        }
        return null;
    }
    ...
    parsePrimitives(primitivesNode) {
        const validPrimitives = ['rectangle', 'triangle', 'cylinder', 'sphere', 'torus', 'patch'];
        const isNumeric = num => num != null && !isNaN(num);
        const unableToParse = (cname, id) => `unable to parse ${cname} of the primitive for ID = ${id}`
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            if (grandChildren.length != 1 || !validPrimitives.includes(grandChildren[0].nodeName))
                return `There must be exactly 1 primitive type (rectangle, triangle, cylinder, sphere, torus, patch)`
            ...
            if (primitiveType == 'rectangle') {
                ...
                let uDegree = this.reader.getInteger(grandChildren[0], 'degree_u');
                if (!isNumeric(uDegree) || uDegree <= 0 || uDegree > 3)
                    return unableToParse('degree_u', primitiveId);
                let uDivs = this.reader.getInteger(grandChildren[0], 'parts_u');
                if (!isNumeric(uDivs) || uDivs <= 0)
                    return unableToParse('parts_u', primitiveId);
                let vDegree = this.reader.getInteger(grandChildren[0], 'degree_v');
                if (!isNumeric(vDegree) || vDegree <= 0 || vDegree > 3)
                    return unableToParse('degree_v', primitiveId);
                let vDivs = this.reader.getInteger(grandChildren[0], 'parts_v');
                if (!isNumeric(vDivs) || vDivs <= 0)
                    return unableToParse('parts_u', primitiveId);

                let controlPointNodes = grandChildren[0].children;
                if (controlPointNodes.length != (uDegree + 1) * (vDegree + 1))
                    return unableToParse('controlpoint', primitiveId);

                let controlPoints = [];
                for (let u = 0; u <= uDegree; u++) {
                    let vPoints = [];

                    for (let v = 0; v <= vDegree; v++) {
                        let controlPointNode = controlPointNodes[u * (vDegree + 1) + v];

                        if (controlPointNode.nodeName !== 'controlpoint') {
                            this.onXMLMinorError(`invalid <controlpoint> in patch primitive of ID = ${primitiveId})`);
                            return null;
                        }

                        let point = this.parseCoordinates3D(controlPointNode);
                        vPoints.push([...point, 1.0]);
                    }

                    controlPoints.push(vPoints);
                }

                let patch = new MyPatch(this.scene, primitiveId, uDegree, vDegree, controlPoints, uDivs, vDivs);
                this.primitives[primitiveId] = patch;

            } else {
                this.onXMLMinorError("unkown primitive type of the primitive for ID = " + primitiveId);
            }
        }
   ...
   */
    parseComponents(componentsNode) {
        ...
        for (let i = 0; i < children.length; i++) {
            ...
            if (highlightedIndex !== -1) {
                ...
                const validScaleFactor = h => h !== null && !isNaN(h) && h > 0;
                const unableToParse = cname => `unable to parse ${cname} from highlighted of component of ID = ${componentID}`;

                let highlighted = grandChildren[highlightedIndex];

                let r = this.reader.getFloat(highlighted, 'r');
                if (!validColorComponent(r))
                    return unableToParse('r');

                let g = this.reader.getFloat(highlighted, 'g');
                if (!validColorComponent(g))
                    return unableToParse('g');

                let b = this.reader.getFloat(highlighted, 'b');
                if (!validColorComponent(b))
                    return unableToParse('b');

                let scaleH = this.reader.getFloat(highlighted, 'scale_h');
                if (!validScaleFactor(scaleH))
                    return unableToParse('scale_h');

                component.highlighted = true;
                this.scene.highlighted.set(componentID, {
                    enabled: true,
                    color: [r, g, b],
                    scale: scaleH
                });
            ...
            if ( animationIndex !== -1){
                ...
                if(this.animations[animationId] == undefined){
                    this.onXMLMinorError(`Invalid id in <animation> tag (conflict: ID = ${animationId})`)
                }
    ...
    parseCoordinates3D(node, messageError) {
        const unableToParse = (cname, messageError) => `unable to parse ${cname} of the ${messageError}`;
        const isNumeric = num => num != null && !isNaN(num);

        let x = this.reader.getFloat(node, 'x');
        if (!isNumeric(x))
            return unableToParse('x-coordinate', messageError);

        let y = this.reader.getFloat(node, 'y');
        if (!isNumeric(y))
            return unableToParse('y-coordinate', messageError);

        let z = this.reader.getFloat(node, 'z');
        if (!isNumeric(z))
            return unableToParse('z-coordinate', messageError);
     ...
     * @param {message to be displayed in case of error} messageError
     */
     parseCoordinates3DVersionS(node, messageError) {
        const unableToParse = (cname, messageError) => `unable to parse ${cname} of the ${messageError}`;
        const isNumeric = num => num != null && !isNaN(num);

        let x = this.reader.getFloat(node, 'sx');
        if (!isNumeric(x))
            return unableToParse('x-coordinate', messageError);

        let y = this.reader.getFloat(node, 'sy');
        if (!isNumeric(y))
            return unableToParse('y-coordinate', messageError);

        let z = this.reader.getFloat(node, 'sz');
        if (!isNumeric(z))
            return unableToParse('z-coordinate', messageError);
    ...
    /**
     * Updates all animations matrix
     * @param {int} elapsedTime
     */
    computeAnimation(elapsedTime) {
        for (const animation of Object.values(this.animations))
            animation.update(elapsedTime / 1000);
    }
    ...
    processComponent(id, latestMaterial, latestTexture, latestLengthS, latestLengthT) {
        ...
        let transfMatrix = component.transformationMatrix;
        ...
        let lengthS = component.lengthS, lengthT = component.lengthT;
        if (textureId === "inherit") {
            textureId = latestTexture;

            if (component.lengthS === null)
                component.lengthS = latestLengthS;
            if (component.lengthT === null)
                component.lengthT = latestLengthT;
        }

        if (textureId === "none")
            textureId = null;

        let currentMaterial = this.materials[materialId];
        currentMaterial.setTexture(this.textures[textureId]);
        currentMaterial.apply();

        let componentPrimitives = component.childrenPrimitives;

        if (this.scene.highlighted.has(id)) {
            const highlighted = this.scene.highlighted.get(id);

            if (highlighted.enabled) {
                this.scene.setActiveShaderSimple(this.scene.highlightShader);
                this.scene.highlightShader.setUniformsValues({
                    scale: highlighted.scale,
                    target: [...highlighted.color, 1.0],
                    uFrontMaterial: currentMaterial,
                });
            }
        }
        ...
        for (let i = 0; i < componentPrimitives.length; i++) {
            if (component.hasToUpdateTexCoords && textureId !== null) {
                this.primitives[componentPrimitives[i]].updateTexCoords(lengthS, lengthT);
